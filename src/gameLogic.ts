import { ICON_IDS } from "./assets/icons";
import type {
  AvailableMatch,
  BoardData,
  DifficultySetting,
...
export function createBoard(setting: DifficultySetting): BoardData {
  ...
  for (let attempt = 0; attempt < 200; attempt += 1) {
    const board = buildBoardFromIcons(
      shuffle(iconIds),
      setting.rows,
      setting.cols
    );

    if (hasAvailableMatch(board)) {
      return board;
    }
  }
...
export function findAvailableMatch(board: BoardData): AvailableMatch | null {
...
export function shuffleRemainingTiles(board: BoardData): {
  board: BoardData;
  ...
  if (positions.length <= 2) {
    ...
    return { board: unchanged, match: findAvailableMatch(unchanged) };
  }

  for (let attempt = 0; attempt < 500; attempt += 1) {
    const next = replaceIcons(board, positions, shuffle(icons));
    const match = findAvailableMatch(next);

    if (match) {
      return { board: next, match };
    }
  }

  const forced = forceAtLeastOneMatch(board, positions, icons);
  return { board: forced, match: findAvailableMatch(forced) };
}

export function isBoardCleared(board: BoardData): boolean {
  return getRemainingTiles(board).length === 0;
}

export function getRemainingTiles(board: BoardData): TileData[] {
  return board.flat().filter((tile) => !tile.removed);
}
...
function replaceIcons(
  ...
  return board.map((row) =>
    row.map((tile) => {
      ...
      return iconId ? { ...tile, iconId } : { ...tile };
    })
  );
}
